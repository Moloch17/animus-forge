...
bool AnimusForge::Curriculum::ActionCatalog::IsCombatSpell(SpellInfo const* info)
{
...
SpellInfo const* AnimusForge::Curriculum::ActionCatalog::KnownRank(Player const* bot, uint32 firstRank)
{
...
AnimusForge::Curriculum::ActionCatalog::ActionCatalog(uint8 playerClass, ClassKit const& kit,
    TalentBuilder const& talents)
{
    ...
    for (uint8 race = RACE_HUMAN; race <= RACE_DRAENEI; ++race)
    {
        ...
        AnimusForge::BotFactory::BotSpec spec;
        spec.Name = Acore::StringFormat("Forgeprobe{}", race);
        spec.Race = race;
        spec.Class = playerClass;
        spec.Gender = GENDER_MALE;
        spec.Level = DEFAULT_MAX_LEVEL;
        spec.AccountId = AnimusForge::BotAccounts::Probe(race);

        Player* probe = AnimusForge::BotFactory::Create(spec);
        if (!probe)
            continue;
        ...
        AnimusForge::BotFactory::DestroyUnplaced(probe);
    }
...
bool AnimusForge::Curriculum::ActionCatalog::IsTacticalSpell(SpellInfo const* info)
{
    if (!info || info->IsPassive())
        return false;
...
bool AnimusForge::Curriculum::ActionCatalog::IsSustainSpell(SpellInfo const* info)
{
    if (!info || info->IsPassive())
        return false;
...
bool AnimusForge::Curriculum::ActionCatalog::IsInterruptingSpell(SpellInfo const* info)
{
    for (SpellEffectInfo const& effect : info->GetEffects())
    {
