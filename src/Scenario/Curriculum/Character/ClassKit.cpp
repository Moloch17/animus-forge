...
AnimusForge::Curriculum::ClassKit::ClassKit(uint8 playerClass) : _class(playerClass)
{
    std::set<uint32> seen;
...
uint8 AnimusForge::Curriculum::ClassKit::MinLevelOf(uint8 playerClass)
{
    return playerClass == CLASS_DEATH_KNIGHT ? DEATH_KNIGHT_START_LEVEL : 1;
}

void AnimusForge::Curriculum::ClassKit::Learn(Player* bot) const
{
...
void AnimusForge::Curriculum::ClassKit::StoreReagents(Player* bot) const
{
...
uint32 AnimusForge::Curriculum::ClassKit::ArmorSubclass(uint8 level) const
{
    switch (_class)
    {
        case CLASS_WARRIOR:
