...
char const* AnimusForge::Curriculum::StageScenario::Name() const
{
    return _stage.Name.c_str();
}

void AnimusForge::Curriculum::StageScenario::AddCoreEpisodeInfo()
{
...
void AnimusForge::Curriculum::StageScenario::WriteStageFiles(ForgeConfig const& config) const
{
...
AnimusForge::Curriculum::EnvState& AnimusForge::Curriculum::StageScenario::Data(Env const& env)
{
    return _data[env.Index];
}

AnimusForge::Curriculum::EnvState const& AnimusForge::Curriculum::StageScenario::Data(Env const& env) const
{
    return _data[env.Index];
}

Player* AnimusForge::Curriculum::StageScenario::SeatBot(Env const& env, uint32 seat) const
{
...
bool AnimusForge::Curriculum::StageScenario::IsTerminal(Env const& env) const
{
...
void AnimusForge::Curriculum::StageScenario::Reset(Env& env)
{
...
bool AnimusForge::Curriculum::StageScenario::Rebuild(Env& env)
{
    EnvState& data = Data(env);
...
Player* AnimusForge::Curriculum::StageScenario::BuildSeat(Env& env, uint32 seatIndex, Map*& map, uint8 level,
...
void AnimusForge::Curriculum::StageScenario::PrepareFighter(Player* bot, SeatState& seat) const
{
...
void AnimusForge::Curriculum::StageScenario::StockSeats(Env& env)
{
    EnvState& data = Data(env);
...
void AnimusForge::Curriculum::StageScenario::AcceptResurrections(Env& env)
{
    EnvState& data = Data(env);
...
bool AnimusForge::Curriculum::StageScenario::DeadForGood(Env const& env, uint32 seatIndex) const
{
...
bool AnimusForge::Curriculum::StageScenario::SeatCanResurrect(Env const& env, uint32 seatIndex) const
{
...
void AnimusForge::Curriculum::StageScenario::NotifyRecovered(Env& env, int32 who)
{
    if (who >= 0)
...
void AnimusForge::Curriculum::StageScenario::ApplyActions(Env& env, int32 const* actions)
{
...
AnimusForge::Curriculum::SeatView AnimusForge::Curriculum::StageScenario::ViewSeat(Env const& env,
...
void AnimusForge::Curriculum::StageScenario::ApplySeatAction(Env& env, uint32 seatIndex, int32 action)
{
    Player* bot = env.FindBot(seatIndex);
    SeatState& seat = Data(env).Seats[seatIndex];
    if (!bot || !seat.L)
...
void AnimusForge::Curriculum::StageScenario::Observe(Env& env, float* obs, float* state, uint8* mask)
{
    for (uint32 seat = 0; seat < _seatCount; ++seat)
...
void AnimusForge::Curriculum::StageScenario::AgentLayouts(Env const& env, uint16* layout) const
{
...
void AnimusForge::Curriculum::StageScenario::AgentPresence(Env const& env, uint8* present) const
{
    EnvState const& data = Data(env);
    for (uint32 seat = 0; seat < _seatCount; ++seat)
...
void AnimusForge::Curriculum::StageScenario::ObserveSeat(Env& env, uint32 seatIndex, float* obs, uint8* mask)
{
...
float AnimusForge::Curriculum::StageScenario::SeatReward(Env& env, uint32 seatIndex)
{
    SeatState& seat = Data(env).Seats[seatIndex];
    if (!seat.L)
...
void AnimusForge::Curriculum::StageScenario::WriteState(Env const& env, float* state) const
{
...
void AnimusForge::Curriculum::StageScenario::EpisodeInfo(Env const& env, float* info) const
{
    for (uint32 seat = 0; seat < _seatCount; ++seat)
        _info.Write(env, seat, info + seat * _spec.EpisodeInfoDim);
}

bool AnimusForge::Curriculum::StageScenario::ScriptedAction(std::string const& policy, float const* obs,
...
void AnimusForge::Curriculum::StageScenario::Teardown(Env& env)
{
    for (uint32 target = 0; target < env.Targets.size(); ++target)
        if (Creature* creature = env.FindTarget(target))
            creature->DespawnOrUnsummon();
