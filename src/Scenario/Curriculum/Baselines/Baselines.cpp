...
bool AnimusForge::Curriculum::Baselines::Supports(std::string const& policy, Layout const& layout)
{
    return policy == "greedy" || (policy == "fight" && layout.Has(BlockId::Duel));
}

int32 AnimusForge::Curriculum::Baselines::Choose(std::string const& policy, Layout const& layout, float const* obs,
    uint8 const* mask)
{
