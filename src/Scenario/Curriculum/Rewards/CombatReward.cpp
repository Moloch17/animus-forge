...
std::string_view AnimusForge::Curriculum::RewardTermName(RewardTerm term)
{
    switch (term)
    {
...
float AnimusForge::Curriculum::CombatReward::DesiredRange(SeatState const& seat,
...
float AnimusForge::Curriculum::CombatReward::TimeLeft(Env const& env)
{
    return env.EpisodeLengthMs
        ? 1.0f - std::min(1.0f, float(env.EpisodeElapsedMs) / float(env.EpisodeLengthMs)) : 0.0f;
}

void AnimusForge::Curriculum::CombatReward::Casting(Player* bot, AgentStats const& step, CombatTally& tally,
...
void AnimusForge::Curriculum::CombatReward::Approach(Player* bot, Unit* target, float desiredRange, float weight,
...
void AnimusForge::Curriculum::CombatReward::OneOnOne(StageScenario& scenario, Env const& env,
