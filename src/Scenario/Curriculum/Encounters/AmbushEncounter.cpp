...
static_assert(AnimusForge::Curriculum::MAX_AMBUSHERS <= AnimusForge::BotAccounts::AMBUSHERS_PER_ENV,
    "every ambusher needs its own bot accounts");

AnimusForge::Curriculum::AmbushEncounter::AmbushEncounter(StageScenario& scenario, uint32 envs)
    : Encounter(scenario), _envs(envs)
{
}

std::vector<AnimusForge::Curriculum::RewardTerm> AnimusForge::Curriculum::AmbushEncounter::RewardTerms() const
{
...
void AnimusForge::Curriculum::AmbushEncounter::AddEpisodeInfo(EpisodeInfoTable& table)
{
...
void AnimusForge::Curriculum::AmbushEncounter::ResetEpisode(Env& env)
{
...
void AnimusForge::Curriculum::AmbushEncounter::BeforeRebuild(Env& env)
{
    // Last episode's ambushers leave before the new characters come.
    RemoveBots(env);
}

bool AnimusForge::Curriculum::AmbushEncounter::Build(Env& env, Map* map, uint8 /*level*/)
{
    EnvAmbush& ambush = _envs[env.Index];
...
bool AnimusForge::Curriculum::AmbushEncounter::Arrive(Env& env, Map* map)
{
    EnvAmbush& ambush = _envs[env.Index];
    ...
    uint32 const envIndex = env.Index;
    uint32 created = 0;
...
void AnimusForge::Curriculum::AmbushEncounter::Update(Env& env)
{
    EnvAmbush& ambush = _envs[env.Index];
    if (!ambush.Arrived)
    {
...
bool AnimusForge::Curriculum::AmbushEncounter::SelectTarget(Env const& env, uint32 seatIndex, Unit*& target)
{
...
void AnimusForge::Curriculum::AmbushEncounter::View(Env const& env, uint32 /*seat*/, SeatView& view) const
{
    uint32 index = 0;
...
void AnimusForge::Curriculum::AmbushEncounter::BeforeRewards(Env& env)
{
    EnvAmbush& ambush = _envs[env.Index];
    ambush.StepKills = 0;
    for (uint32 index = 0; index < ambush.Count; ++index)
...
void AnimusForge::Curriculum::AmbushEncounter::Reward(Env& env, uint32 seat, Player* bot, RewardLedger& ledger)
{
    if (Alone(env))
    {
...
bool AnimusForge::Curriculum::AmbushEncounter::IsTerminal(Env const& env) const
{
    if (!Alone(env))
        return false;
...
Player* AnimusForge::Curriculum::AmbushEncounter::Find(Env const& env, uint32 ambusher) const
{
...
Player* AnimusForge::Curriculum::AmbushEncounter::FirstAlive(Env const& env, uint32* index) const
{
...
void AnimusForge::Curriculum::AmbushEncounter::RemoveBots(Env& env)
{
    EnvAmbush& ambush = _envs[env.Index];
    for (Ambusher& ambusher : ambush.Ambushers)
    {
...
void AnimusForge::Curriculum::AmbushEncounter::Deactivate(Env& env)
{
    RemoveBots(env);
...
void AnimusForge::Curriculum::AmbushEncounter::Teardown(Env& env)
{
    RemoveBots(env);
}
