...
AnimusForge::Curriculum::PartyEncounter::PartyEncounter(StageScenario& scenario, uint32 envs)
    : Encounter(scenario), _envs(envs)
{
}

std::vector<AnimusForge::Curriculum::RewardTerm> AnimusForge::Curriculum::PartyEncounter::RewardTerms() const
{
    return { RewardTerm::TeammateDamageTaken, RewardTerm::TeammateHealing, RewardTerm::TeammateThreat,
        RewardTerm::TeammateDeath };
}

void AnimusForge::Curriculum::PartyEncounter::AddEpisodeInfo(EpisodeInfoTable& table)
{
...
void AnimusForge::Curriculum::PartyEncounter::ResetEpisode(Env& env)
{
    _envs[env.Index].Seats.fill(SeatParty());
}

Player* AnimusForge::Curriculum::PartyEncounter::Tank(Env const& env) const
{
...
void AnimusForge::Curriculum::PartyEncounter::BeforeRebuild(Env& env)
{
    // The old party goes before its members do.
    Disband(env);
}

bool AnimusForge::Curriculum::PartyEncounter::Build(Env& env, Map* /*map*/, uint8 /*level*/)
{
    EnvState const& data = _scenario.Data(env);
...
void AnimusForge::Curriculum::PartyEncounter::Disband(Env& env)
{
    EnvParty& party = _envs[env.Index];
    if (!party.PartyGroup)
        return;
...
void AnimusForge::Curriculum::PartyEncounter::View(Env const& env, uint32 seatIndex, SeatView& view) const
{
    EnvState const& data = _scenario.Data(env);
...
void AnimusForge::Curriculum::PartyEncounter::Reward(Env& env, uint32 seatIndex, Player* bot, RewardLedger& ledger)
{
    if (!bot)
        return;
...
void AnimusForge::Curriculum::PartyEncounter::OnRecovered(Env& env, int32 who)
{
    if (who < 0)
        return;
...
void AnimusForge::Curriculum::PartyEncounter::Teardown(Env& env)
{
    Disband(env);
}
