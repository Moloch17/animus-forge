...
AnimusForge::Curriculum::OwnerEncounter::OwnerEncounter(StageScenario& scenario, uint32 envs)
    : Encounter(scenario), _envs(envs)
{
}

Player* AnimusForge::Curriculum::OwnerEncounter::Find(Env const& env) const
{
    return _envs[env.Index].Bot.Active();
}

std::vector<AnimusForge::Curriculum::RewardTerm> AnimusForge::Curriculum::OwnerEncounter::RewardTerms() const
{
...
void AnimusForge::Curriculum::OwnerEncounter::AddEpisodeInfo(EpisodeInfoTable& table)
{
...
void AnimusForge::Curriculum::OwnerEncounter::ResetEpisode(Env& env)
{
...
bool AnimusForge::Curriculum::OwnerEncounter::Build(Env& env, Map* map, uint8 level)
{
    EnvOwner& owner = _envs[env.Index];
...
void AnimusForge::Curriculum::OwnerEncounter::Update(Env& env)
{
...
void AnimusForge::Curriculum::OwnerEncounter::View(Env const& env, uint32 /*seat*/, SeatView& view) const
{
    view.Owner = Find(env);
}

void AnimusForge::Curriculum::OwnerEncounter::BeforeRewards(Env& env)
{
...
void AnimusForge::Curriculum::OwnerEncounter::Reward(Env& env, uint32 seatIndex, Player* bot, RewardLedger& ledger)
{
    EnvOwner& state = _envs[env.Index];
    Player* owner = Find(env);
...
void AnimusForge::Curriculum::OwnerEncounter::WriteState(Env const& env, float* state) const
{
    Player* owner = Find(env);
...
void AnimusForge::Curriculum::OwnerEncounter::OnRecovered(Env& env, int32 who)
{
...
void AnimusForge::Curriculum::OwnerEncounter::OnPullStarting(Env& env)
{
    Player* owner = Find(env);
    if (!owner)
        return;
