...
AnimusForge::Curriculum::BlockSize AnimusForge::Curriculum::CompanionBlock::Size(Layout const& layout) const
{
...
void AnimusForge::Curriculum::CompanionBlock::DescribeManifest(Layout const& layout, boost::json::object& block) const
{
    block["ally_heals"] = SpellList(layout.AllyHeals);
    block["ally_revives"] = SpellList(layout.AllyRevives);
}

void AnimusForge::Curriculum::CompanionBlock::Observe(SeatView const& view, float* obs, uint8* mask) const
{
    Player* bot = view.Bot;
    Player* owner = view.Owner;
...
void AnimusForge::Curriculum::CompanionBlock::Apply(SeatView& view, uint32 local, SeatActionResult& result) const
{
