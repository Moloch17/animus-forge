...
AnimusForge::Curriculum::BlockSize AnimusForge::Curriculum::GauntletBlock::Size(Layout const& layout) const
{
    uint32 const sustain = uint32(layout.Catalog().Sustain().size());
    return { OBS_GLOBAL_COUNT + sustain * 2, ACTION_SUSTAIN_FIRST + sustain };
}

void AnimusForge::Curriculum::GauntletBlock::DescribeManifest(Layout const& layout, boost::json::object& block) const
{
    block["consumables"] = CONSUMABLE_COUNT;
    block["sustain"] = SpellList(layout.Catalog().Sustain());
}

void AnimusForge::Curriculum::GauntletBlock::Observe(SeatView const& view, float* obs, uint8* mask) const
{
    Player* bot = view.Bot;
...
void AnimusForge::Curriculum::GauntletBlock::Apply(SeatView& view, uint32 local, SeatActionResult& result) const
{
