...
AnimusForge::Curriculum::BlockSize AnimusForge::Curriculum::CoreBlock::Size(Layout const& layout) const
{
...
uint32 AnimusForge::Curriculum::CoreBlock::TalentObsFirst(Layout const& layout)
{
    return layout.Slice(BlockId::Core).ObsFirst + OBS_GLOBAL_COUNT
        + uint32(layout.Catalog().Actions().size()) * ACTION_FEATURES;
}

uint32 AnimusForge::Curriculum::CoreBlock::TreeObsFirst(Layout const& layout)
{
    return TalentObsFirst(layout) + uint32(layout.Assets->Talents->Talents().size());
}

void AnimusForge::Curriculum::CoreBlock::DescribeManifest(Layout const& layout, boost::json::object& block) const
{
...
void AnimusForge::Curriculum::CoreBlock::ObserveCharacter(SeatView const& view, float* obs)
{
...
void AnimusForge::Curriculum::CoreBlock::Observe(SeatView const& view, float* obs, uint8* mask) const
{
    Player* bot = view.Bot;
...
void AnimusForge::Curriculum::CoreBlock::Apply(SeatView& view, uint32 local, SeatActionResult& result) const
{
