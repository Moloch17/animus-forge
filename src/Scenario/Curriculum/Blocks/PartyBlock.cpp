...
namespace
{
    ...
    bool IsAllowed(SeatView const& view, uint32 action)
    {
        ...
        if (action < PartyBlock::ACTION_HEAL_FIRST)
        {
...
AnimusForge::Curriculum::BlockSize AnimusForge::Curriculum::PartyBlock::Size(Layout const& layout) const
{
...
void AnimusForge::Curriculum::PartyBlock::DescribeManifest(Layout const& /*layout*/, boost::json::object& block) const
{
    block["members"] = PARTY_MEMBERS;
    block["member_features"] = uint32(MEMBER_FEATURES);
}

void AnimusForge::Curriculum::PartyBlock::Observe(SeatView const& view, float* obs, uint8* mask) const
{
    Player* bot = view.Bot;
...
void AnimusForge::Curriculum::PartyBlock::Apply(SeatView& view, uint32 local, SeatActionResult& result) const
{
