...
AnimusForge::ClassRole::PartyEncounter::PartyEncounter(ClassRoleScenario& scenario, uint32 envs)
    : Encounter(scenario), _envs(envs)
{
}

std::vector<AnimusForge::ClassRole::RewardTerm> AnimusForge::ClassRole::PartyEncounter::RewardTerms() const
{
    return { RewardTerm::TeammateDamageTaken, RewardTerm::TeammateHealing, RewardTerm::TeammateThreat,
        RewardTerm::TeammateDeath };
}

void AnimusForge::ClassRole::PartyEncounter::AddEpisodeInfo(EpisodeInfoTable& table)
{
...
void AnimusForge::ClassRole::PartyEncounter::ResetEpisode(Env& env)
{
    _envs[env.Index].Seats.fill(SeatParty());
}

Player* AnimusForge::ClassRole::PartyEncounter::Tank(Env const& env) const
{
...
void AnimusForge::ClassRole::PartyEncounter::BeforeRebuild(Env& env)
{
    // The old party goes before its members do.
    Disband(env);
}

bool AnimusForge::ClassRole::PartyEncounter::Build(Env& env, Map* /*map*/, uint8 /*level*/)
{
    EnvState const& data = _scenario.Data(env);
...
void AnimusForge::ClassRole::PartyEncounter::Disband(Env& env)
{
    EnvParty& party = _envs[env.Index];
    if (!party.PartyGroup)
        return;
...
void AnimusForge::ClassRole::PartyEncounter::View(Env const& env, uint32 seatIndex, SeatView& view) const
{
    EnvState const& data = _scenario.Data(env);
...
void AnimusForge::ClassRole::PartyEncounter::Reward(Env& env, uint32 seatIndex, Player* bot, RewardLedger& ledger)
{
    if (!bot)
        return;

    ClassRoleTuning::PartyTuning const& tuning = _scenario.Tuning().Party;
    EnvState const& data = _scenario.Data(env);
...
void AnimusForge::ClassRole::PartyEncounter::OnRecovered(Env& env, int32 who)
{
    if (who < 0)
        return;
...
void AnimusForge::ClassRole::PartyEncounter::Teardown(Env& env)
{
    Disband(env);
}
