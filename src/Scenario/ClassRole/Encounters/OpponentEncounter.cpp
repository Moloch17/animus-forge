...
AnimusForge::ClassRole::OpponentEncounter::OpponentEncounter(ClassRoleScenario& scenario, uint32 envs, bool mirror)
    : Encounter(scenario), _mirror(mirror), _envs(envs)
{
}

std::vector<AnimusForge::ClassRole::RewardTerm> AnimusForge::ClassRole::OpponentEncounter::RewardTerms() const
{
...
void AnimusForge::ClassRole::OpponentEncounter::AddEpisodeInfo(EpisodeInfoTable& table)
{
...
Player* AnimusForge::ClassRole::OpponentEncounter::Find(Env const& env, uint32 seat) const
{
    if (_mirror)
...
bool AnimusForge::ClassRole::OpponentEncounter::Build(Env& env, Map* map, uint8 /*level*/)
{
...
bool AnimusForge::ClassRole::OpponentEncounter::RebuildScripted(Env& env, Player* bot, Map* map)
{
    EnvOpponent& opponent = _envs[env.Index];
    ClassRoleTuning::OpponentTuning const& tuning = _scenario.Tuning().Opponent;
...
void AnimusForge::ClassRole::OpponentEncounter::Update(Env& env)
{
...
bool AnimusForge::ClassRole::OpponentEncounter::SelectTarget(Env const& env, uint32 seat, Unit*& target)
{
    target = Find(env, seat);
    return true;
}

void AnimusForge::ClassRole::OpponentEncounter::View(Env const& env, uint32 seat, SeatView& view) const
{
...
void AnimusForge::ClassRole::OpponentEncounter::Reward(Env& env, uint32 seat, Player* bot, RewardLedger& ledger)
{
...
bool AnimusForge::ClassRole::OpponentEncounter::IsTerminal(Env const& env) const
{
...
void AnimusForge::ClassRole::OpponentEncounter::Teardown(Env& env)
{
    if (!_mirror)
