...
bool AnimusForge::ClassRole::ActionCatalog::IsCombatSpell(SpellInfo const* info)
{
...
SpellInfo const* AnimusForge::ClassRole::ActionCatalog::KnownRank(Player const* bot, uint32 firstRank)
{
...
AnimusForge::ClassRole::ActionCatalog::ActionCatalog(uint8 playerClass, ClassKit const& kit, TalentBuilder const& talents)
{
    ...
    for (uint8 race = RACE_HUMAN; race <= RACE_DRAENEI; ++race)
    {
        ...
        spec.AccountId = AnimusForge::TrainingDummyArena::BOT_ACCOUNT_BASE - 1 - race;
...
bool AnimusForge::ClassRole::ActionCatalog::IsSustainSpell(SpellInfo const* info)
{
    if (!info || info->IsPassive())
        return false;
...
bool AnimusForge::ClassRole::ActionCatalog::IsInterruptingSpell(SpellInfo const* info)
{
    for (SpellEffectInfo const& effect : info->GetEffects())
    {
