...
void AnimusForge::Forge::CommandStatus(LineSink const& out)
{
    ...
    else
    {
        ...
        table.AddRow({ "queue (forge start)", _config.Queue.empty() ? "-" : Join(_config.Queue) });
...
void AnimusForge::Forge::CommandScenarios(LineSink const& out)
{
    ...
    for (std::string const& name : ScenarioNames())
    {
        ...
        if (_state != State::Idle && name == _config.Scenario)
            state = StateName();
...
bool AnimusForge::Forge::CommandStart(std::vector<std::string> scenarios, LineSink const& out)
{
    ...
    {
        out(Acore::StringFormat("{} is {}: `forge cancel` it first.", _config.Scenario, StateName()));
        return false;
    }

    if (scenarios.empty())
        scenarios = _config.Queue;

    if (scenarios.empty())
    {
        out("Nothing to start: name the scenarios (`forge start <scenario> [scenario ...]`) or set AnimusForge.Queue.");
        return false;
    }
...
bool AnimusForge::Forge::CommandResume(std::vector<std::string> scenarios, LineSink const& out)
{
    ...
    {
        if (!scenarios.empty() && (scenarios.size() > 1 || scenarios.front() != _config.Scenario))
        {
            out(Acore::StringFormat("{} is paused: `forge resume` continues it; `forge cancel` it to run something "
                "else.", _config.Scenario));
            return false;
        }

        _resumeRequested = true;
        out(Acore::StringFormat("Resuming {}.", _config.Scenario));
        return true;
    }
    ...
    if (_state == State::Training && !_server.HasClient() && !_learner.IsRunning() && _config.LearnerAutoStart)
    {
        if (!scenarios.empty() && (scenarios.size() > 1 || scenarios.front() != _config.Scenario))
        {
            out(Acore::StringFormat("{} is waiting for its learner: `forge resume` restarts it, `forge cancel` stops "
                "the plan.", _config.Scenario));
            return false;
        }

        std::error_code error;
        bool const resume = fs::exists(_config.RunsDir() / _config.Scenario / "latest.pt", error);
        _plan.Entries[_plan.Index].Resume = resume;
        _learnerStarted = _learner.Start(_config, _config.Scenario, resume);
        if (!_learnerStarted)
        {
            out(Acore::StringFormat("Could not restart the learner for {}; see the server log.", _config.Scenario));
            return false;
        }

        out(Acore::StringFormat("Restarted the learner for {}{}.", _config.Scenario,
    ...
    if (_state != State::Idle || _request == Request::Start)
    {
        out(Acore::StringFormat("{} is {}: `forge pause` pauses it, `forge cancel` stops it.", _config.Scenario,
            StateName()));
        return false;
    }
...
bool AnimusForge::Forge::CommandPause(LineSink const& out)
{
    ...
    {
        out(Acore::StringFormat("{} is already paused; `forge resume` continues it.", _config.Scenario));
        return false;
    }
    ...
    out(Acore::StringFormat("Pausing {} after the current decision.", _config.Scenario));
    return true;
}
...
bool AnimusForge::Forge::CommandCancel(LineSink const& out)
{
    ...
    out(Acore::StringFormat("Cancelling {}{}.", _config.Scenario, _plan.Remote()
...
bool AnimusForge::Forge::CommandSkip(LineSink const& out)
{
    ...
    out(Acore::StringFormat("Skipping {}{}; {}.", _config.Scenario,
...
bool AnimusForge::Forge::CommandClean(std::string const& target, std::string const& scenario, LineSink const& out)
{
    ...
    else if (target == "scenario")
    {
        ...
        if ((_state != State::Idle && scenario == _config.Scenario) || (_request == Request::Start && requested))
        {
