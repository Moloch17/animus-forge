...
def test_writer_keeps_metrics_and_evaluation_between_writes(tmp_path):
    config = _config()
    spec = SimpleNamespace(scenario="stage1_duel")
...
def _config() -> TrainConfig:
    config = TrainConfig(run_name="stage1_duel", total_env_steps=1000)
...
def test_restore_evaluation_after_resume(tmp_path):
    ...
    writer = ProgressWriter(tmp_path, _config(), SimpleNamespace(scenario="stage1_duel"))
...
def test_restarts_and_target_are_reported(tmp_path):
    ...
    writer = ProgressWriter(tmp_path, config, SimpleNamespace(scenario="stage1_duel"))
