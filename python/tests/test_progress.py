...
from animus.evaluation import PlateauTracker
...
def test_writer_keeps_metrics_and_evaluation_between_writes(tmp_path):
    config = _config()
    spec = SimpleNamespace(scenario="class_role_duel")
    tracker = PlateauTracker(patience=3)
...
def _config() -> TrainConfig:
    config = TrainConfig(run_name="class_role_duel", total_env_steps=1000)
    config.eval.every_env_steps = 100
    config.eval.baseline = "fight"
    config.plateau.patience = 3
    config.plateau.min_env_steps = 200
    return config


def test_restore_evaluation_after_resume(tmp_path):
    tracker = PlateauTracker(patience=2)
    tracker.observe(3.0, 100)
    tracker.observe(2.0, 200)
    writer = ProgressWriter(tmp_path, _config(), SimpleNamespace(scenario="class_role_duel"))
